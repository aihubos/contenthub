...
export type Idea = { image?: {src: string; alt: string; caption: string; sourceUrl?: string}; id: string; format: Format; category: string; title: string; summary: string; whyNow: string; fit: string; signal: '시의성' | '채널 적합' | '후속 기획'; duration: string; hook: string; outline: string[]; keywords: string[]; sources: Source[]; duplicate: {type: 'new' | 'followup'; related: string[]; note: string}; guardrail: string; accent: string };
